...
class MT5Executor:
    ...
    def connect(self) -> bool:
        ...
        if not mt5.initialize():
            ...
            return False
        # If the terminal is already logged in, use the existing session
        info = mt5.account_info()
        if info is not None:
            logger.info(
                f"MT5 connected (existing session): account {info.login}  "
                f"equity={info.equity:,.2f}  server={info.server}"
            )
            return True
        # Terminal running but not logged in — authenticate now
